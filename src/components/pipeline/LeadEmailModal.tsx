...
<p style="color: #475569;">Our team will get back to you shortly at <strong>{{contactEmail}}</strong>.</p>`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl border border-base-c bg-card-c p-6 shadow-2xl space-y-5">
        
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-base-c pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-primary-c">Customer Lead Email Template</h3>
                <span className="rounded-full border border-indigo-300 bg-indigo-50 px-2 py-0.5 text-[10px] font-bold text-indigo-700 dark:border-indigo-800 dark:bg-indigo-950/50 dark:text-indigo-300 flex items-center gap-1">
                  <Code className="h-3 w-3" /> HTML & CSS Supported
                </span>
              </div>
              <p className="text-xs text-muted-c">Customize the automated email sent to leads with rich text, HTML markup, and inline CSS styles</p>
            </div>
          </div>

          <div className="flex items-center gap-2 shrink-0">
            {/* Tab Switcher */}
            <div className="flex items-center rounded-lg border border-base-c bg-slate-100 p-0.5 dark:bg-slate-800">
              <button
                type="button"
                onClick={() => setActiveTab('edit')}
                className={cx('flex items-center gap-1 rounded-md px-2.5 py-1 text-xs font-semibold transition-all', activeTab === 'edit' ? 'bg-white text-primary-700 shadow-sm dark:bg-slate-700 dark:text-white' : 'text-muted-c hover:text-primary-c')}
              >
              ...
              <button
                ...
                className={cx('flex items-center gap-1 rounded-md px-2.5 py-1 text-xs font-semibold transition-all', activeTab === 'preview' ? 'bg-white text-primary-700 shadow-sm dark:bg-slate-700 dark:text-white' : 'text-muted-c hover:text-primary-c')}
              >
                <Eye className="h-3 w-3" /> Live HTML Preview
              </button>
            </div>

            <button
              onClick={onClose}
              className="grid h-8 w-8 place-items-center rounded-lg text-muted-c hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
        ...
        {loading ? (
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-3.5 dark:border-amber-800/50 dark:bg-amber-950/20">
              <div className="flex items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className="text-[11px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-400">Dynamic Placeholders & HTML Support</p>
                </div>
                <button
                  type="button"
                  onClick={insertSampleHtml}
                  className="flex items-center gap-1 rounded-lg border border-amber-300 bg-white px-2 py-0.5 text-[10px] font-bold text-amber-800 hover:bg-amber-100 dark:bg-amber-900/40 dark:text-amber-300 transition-all"
                  title="Insert rich HTML sample with inline styles"
                >
                  <Code className="h-3 w-3" /> Load Styled HTML Sample
                </button>
              </div>
            ...
            {/* Controls */}
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-base-c pt-4">
              <button
                type="button"
